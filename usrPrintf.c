...
static size_t strLength(const char *s, size_t count)
{
	const char *sc;
...
int usrVsprintf(char *buf, const char *fmt, va_list args)
{
	int len;
	...
	repeat:
		...
		case 's':
			...
			if (!s)
				s = "";
			len = strLength(s, precision); /*取字符串的长度，最大为precision*/
			if (!(flags & LEFT))
		...
		case 'E': /* Floating point (E) */
			...
			if (!(flags & LEFT))
			{
				for (j = strLength(sprint_fe, SZ_NUM_BUF); j < field_width; j++)
					*str++ = '0';
			}
			i = 0;
			while (sprint_fe[i])
...
{
	int n = 0;
	char sprint_buf[BUFF_LENGTH] = {0};
	memset(sprint_buf, 0, sizeof(sprint_buf));
	va_list args;
	va_start(args, fmt);
	n = usrVsprintf(sprint_buf, fmt, args);
	va_end(args);
	puts(sprint_buf);
	return n;
}

int usr_sprint(char *buf, const char *fmt, ...)
{
	va_list args;
	int i;
	va_start(args, fmt);
	i = usrVsprintf(buf, fmt, args);
	va_end(args);
	return i;
}
...
void logPrint(const char *fileName, const char *funcName, const int lineNum, const char *fmt, ...)
{
	...
	int length = usr_sprint(buf, "%s:%s:%d:", fileName, funcName, lineNum);
	if (2 > length)
	{
		return;
	}
